...
def add_pos(x):
    b, c, h, w = x.shape
    h_coords = torch.linspace(0,1,h).view(1,1,h,1).repeat(b,1,1,w).to(x.device)
    w_coords = torch.linspace(0,1,w).view(1,1,1,w).repeat(b,1,h,1).to(x.device)
    x = torch.cat((x, h_coords, w_coords), 1)
    return x
...
class PointDiTBlock(nn.Module):
    ...
    A DiT block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
    def __init__(self, hidden_size, pos_hidden_size, num_heads, mlp_ratio=4.0, **block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = Attention(hidden_size, num_heads=num_heads, qkv_bias=True, **block_kwargs)
        self.norm2 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        mlp_hidden_dim = int(hidden_size * mlp_ratio)
        approx_gelu = lambda: nn.GELU(approximate="tanh")
        self.mlp = Mlp(in_features=hidden_size+pos_hidden_size, hidden_features=mlp_hidden_dim, out_features=hidden_size, act_layer=approx_gelu, drop=0)
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
            nn.Linear(hidden_size, 6 * hidden_size, bias=True)
        )
    ...
    def forward(self, x, p, c):
        ...
        x = x + gate_mlp.unsqueeze(1) * self.mlp(torch.cat((modulate(self.norm2(x), shift_mlp, scale_mlp), p), -1))
        return x
...
class PointDiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_embedder = PatchEmbed(input_size, patch_size, in_channels, hidden_size, bias=True)
        self.p_embedder = PatchEmbed(input_size, patch_size, pos_channels, pos_hidden_size, bias=True)
        self.t_embedder = TimestepEmbedder(hidden_size)
        self.y_embedder = LabelEmbedder(num_classes, hidden_size, class_dropout_prob)
        num_patches = self.x_embedder.num_patches
        # # Will use fixed sin-cos embedding:
        ...
        self.blocks = nn.ModuleList([
            PointDiTBlock(hidden_size, pos_hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = SimpleFinalLayer(hidden_size, patch_size, self.out_channels)
        self.initialize_weights()
    ...
    def initialize_weights(self):
        ...
        w = self.p_embedder.proj.weight.data
        nn.init.xavier_uniform_(w.view([w.shape[0], -1]))
    ...
    def forward(self, x, t, y):
        ...
        """
        # x = self.x_embedder(x) + self.pos_embed  # (N, T, D), where T = H * W / patch_size ** 2
        p = self.p_embedder(x[:,-2:])            # (N, T, D), where T = H * W / patch_size ** 2, p <- x[:, -2:]
        x = self.x_embedder(x)                   # (N, T, D), where T = H * W / patch_size ** 2
        t = self.t_embedder(t)                   # (N, D)
        y = self.y_embedder(y, self.training)    # (N, D)
        c = t + y                                # (N, D)
